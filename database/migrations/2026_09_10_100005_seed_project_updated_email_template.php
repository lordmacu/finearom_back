...
{
    /**
     * Template del correo de modificación de un proyecto (botón "Enviar
     * modificación"). Solo se inserta si no existe: después se administra
     * desde /settings/email-templates y un deploy no debe pisar esas ediciones.
     */
    public function up(): void
    {
        $exists = DB::table('email_templates')->where('key', 'project_updated')->exists();
        if ($exists) return;

        DB::table('email_templates')->insert([
            'key'  => 'project_updated',
            'name' => 'Proyecto: modificación (responde en el hilo interno)',

            'subject' => 'Actualización proyecto #|project_id| — |project_name| · |client_name|',

            'title' => 'Proyecto actualizado',

            'header_content' => '
<p>Hola equipo,</p>
<p><strong>|created_by|</strong> actualizó el proyecto <strong>|project_name|</strong> para <strong>|client_name|</strong>.
Estos fueron los cambios desde el último correo:</p>
...
',
  ...
  Correo automático de la plataforma Finearom. La lista de destinatarios se administra en Configuración → Procesos ("Proyecto: modificación"; si está vacía, se usa la de "Proyecto: creación").
</p>',
    ...
    public function down(): void
    {
        ...
        DB::table('email_templates')->where('key', 'project_updated')->delete();
    }
};
