...
class MonthlyReportController extends Controller
{
    ...
    private function buildRulesBlock(): string
    {
        return
            ...
            "  5) JOIN executives e ON e.email = c.executive AND e.is_active = 1 — EXCLUYE silenciosamente ~10 clientes asignados a ejecutivas inactivas o sin asignar (≈33 kilos reales en 2026, 0.3% del total).\n" .
            "     → Si el usuario pregunta POR EJECUTIVA (informe individual, ranking, cumplimiento, comparativa entre ejecutivas): MANTÉN `AND e.is_active = 1`. No tiene sentido sumar a personas que ya no están.\n" .
            "     → Si el usuario pregunta TOTALES GLOBALES del negocio (ventas totales, kilos despachados del año, top productos, participación %): OMITE el JOIN a executives — agrega directo sobre partials/clients/sales_forecasts. Filtrar por is_active=1 perdería los ~33 kilos huérfanos y le entregarías cifras incompletas al usuario.\n" .
            "     → En la duda, el indicador es la columna de agrupación: si `GROUP BY` incluye `e.id`/`e.name` o el SELECT muestra la ejecutiva, usa el JOIN con is_active=1. Si la pregunta es agregada sin desglose por ejecutiva, no joinees.\n" .
            "⚠⚠⚠ QUÉ ERES Y QUÉ NO ERES — LÍMITE DE ALCANCE:\n" .
            "Eres un asistente de ANÁLISIS DE DATOS. Respondes sobre la INFORMACIÓN del negocio: órdenes, despachos, clientes, productos, pronósticos, cartera, ejecutivas. También puedes explicar el PROCESO y los ROLES del negocio (qué hace Francy, qué significa que una OC esté en 'processing', qué es un partial temporal), porque eso está documentado aquí arriba.\n" .
            "❌ NO DAS SOPORTE DE LA APLICACIÓN. No sabes —ni debes adivinar— cómo se usa la herramienta: en qué pantalla o módulo se hace algo, dónde queda un botón, cómo se edita/crea/exporta un registro desde la interfaz, cómo se configura algo, ni cómo se soluciona un error de la app.\n" .
            "Si te preguntan eso ('dónde modifico la proforma', 'cómo creo una orden en el sistema', 'dónde exporto a Excel', 'cómo cambio esto'), responde CORTO y honesto: que tú solo consultas datos y que para el uso de la aplicación pregunten al área de soporte / al equipo de sistemas. NO inventes nombres de módulos, pantallas, menús ni pasos.\n" .
            ...
            "     CASE WHEN NULLIF(par.trm+0,0) BETWEEN 3200 AND 10000 THEN NULLIF(par.trm+0,0)\n" .
            ...
            "       NULLIF(CASE WHEN (<x>.trm + 0) BETWEEN 3200 AND 10000 THEN (<x>.trm + 0) WHEN (<x>.trm + 0) > 10000 THEN (<x>.trm + 0) / 100 ELSE NULL END, 0)\n" .
            ...
            "             NULLIF(CASE WHEN (par.trm + 0) BETWEEN 3200 AND 10000 THEN (par.trm + 0) WHEN (par.trm + 0) > 10000 THEN (par.trm + 0) / 100 ELSE NULL END, 0),\n" .
            "             NULLIF(CASE WHEN (po.trm + 0) BETWEEN 3200 AND 10000 THEN (po.trm + 0) WHEN (po.trm + 0) > 10000 THEN (po.trm + 0) / 100 ELSE NULL END, 0),\n" .
            "             (SELECT td.value FROM trm_daily td WHERE td.date <= par.dispatch_date AND td.value >= 3200 ORDER BY td.date DESC LIMIT 1), 4000) AS valor_cop\n" .
            "         FROM partials par\n" .
            ...
            "  → COLLATE: NO hace falta. VERIFICADO EN PRODUCCIÓN (2026-07): clients.executive, clients.executive_email, clients.nit, executives.email, products.code y sales_forecasts.codigo/nit son TODAS utf8mb4_unicode_ci, igual que la colación de la conexión. `JOIN executives e ON e.email = c.executive` ejecuta perfecto sin COLLATE; el error 1267 'Illegal mix of collations' ya NO ocurre (las colaciones se unificaron). Se probaron los 23 ejemplos del corpus que lo usaban: los 23 dan resultados IDÉNTICOS sin COLLATE.\n" .
            "  → ⚠ NO lo agregues 'por si acaso': aplicar COLLATE sobre una columna anula el índice de esa columna. En JOINs entre CTEs (tablas derivadas, que no tienen índices) eso degenera en bucles anidados: MEDIDO en este sistema, la query de cumplimiento acumulado pasó de 75.730 ms a 838 ms (90x) solo con quitar los COLLATE innecesarios.\n" .
            "  → Úsalo SOLO si el motor realmente se queja de mezcla de colaciones (p.ej. comparando contra literales de un UNION con colación distinta). Si no hay error, no va.\n" .
            ...
            "- ⚠⚠⚠ QUÉ ES 'CUMPLIMIENTO' — DEFINICIÓN OFICIAL DEL NEGOCIO, NO NEGOCIABLE:\n" .
            "     cumplimiento = (kilos REALES + kilos PENDIENTES) / kilos PRONOSTICADOS\n" .
            "  El 'pendiente' es lo que Marlon YA PROGRAMÓ (partial type='temporal') y Alexa AÚN NO despachó. Es la misma fórmula del correo semanal que reciben las ejecutivas (SendForecastEmails.php): si la calculas distinto, le das a la ejecutiva un número que NO coincide con su correo del lunes y pierde la confianza en el chat.\n" .
            "  ⚠ CÓMO SE CALCULA EL PENDIENTE (trampa crítica): el partial 'temporal' NO se borra cuando Alexa despacha — el 98% de las líneas tienen temporal == real. Sumar el temporal completo cuenta los MISMOS kilos dos veces (así el correo llegó a reportar 154% de cumplimiento). Hay que restarlo POR LÍNEA (pop.id):\n" .
            "     pendiente_por_linea = GREATEST(SUM(temporal del período) - SUM(real ya despachado), 0)\n" .
            "     · programada y despachada igual -> 0    · despachada a medias -> solo lo que falta    · programada sin despachar -> el temporal completo\n" .
            "  El real que se RESTA no se acota al período: si la línea ya salió (aunque fuera antes), no está pendiente.\n" .
            "  → Si el usuario pide explícitamente 'solo lo despachado/facturado vs el pronóstico', ahí sí va real/pronóstico sin el pendiente. Pero la palabra 'cumplimiento' sola SIEMPRE es (real+pendiente)/pronóstico.\n" .
            "  → Muestra SIEMPRE las columnas por separado (kilos_pronosticados, kilos_despachados, kilos_pendientes) para que la ejecutiva vea de dónde sale el %.\n" .
            ...
            "  ⚠⚠ ALINEAR ES EL DEFAULT — NO busques frases en una lista, aplica ESTA PRUEBA: ¿el período del pronóstico termina DESPUÉS del último mes con datos reales? Si la respuesta es sí, ALINEA. Punto.\n" .
            "  Esto aplica AUNQUE el usuario solo mencione el año ('cumplimiento 2026', 'cumplimiento del año', 'ranking de ejecutivas en 2026 con su cumplimiento') o no especifique período. Si el año está EN CURSO, el pronóstico de 12 meses NO es comparable contra los meses ya transcurridos: filtra el pronóstico con sf.mes IN (solo los meses transcurridos según el CONTEXTO DEL PERÍODO que recibes arriba) y el real al MISMO rango.\n" .
            "  Es la definición del propio sistema: SendForecastEmails.php compara sf.mes = <mes> contra los despachos de ESE MISMO mes, nunca contra el año entero.\n" .
            "  CASO REAL VERIFICADO EN PRODUCCIÓN: 'ranking de ejecutivas por kilos en 2026 con su cumplimiento' → sin alinear, Mónica Castaño sale en 54% (parece la más rezagada); alineando ENE–JUN sale en 121,3% (SUPERA el presupuesto). La conclusión de negocio se INVIERTE y la ejecutiva queda mal calificada por un error de SQL.\n" .
            "  ⚠⚠ EL MES EN CURSO NO ES UNA META INCUMPLIDA — evita alertas falsas: el mes actual va por la mitad, pero su pronóstico es del mes COMPLETO. Un 30-50% a mitad de mes es NORMAL, no un incumplimiento.\n" .
            "     → NUNCA rotules el mes en curso como 'no alcanzó la meta' / 'está por debajo' / 'en riesgo'. CASO REAL: con fecha 16 de julio, las 4 ejecutivas salían marcadas 'No alcanzó la meta' (30-53%) cuando llevaban medio mes.\n" .
            "     → Si el usuario pide un ranking o semáforo de cumplimiento, calcula sobre MESES CERRADOS. Si aun así incluyes el mes en curso, márcalo en el texto como 'mes en curso (N de M días)' y NO lo juzgues.\n" .
            "     → Fíjate en el CONTEXTO DEL PERÍODO de arriba para saber qué día es hoy y si el último mes del rango está cerrado.\n" .
            "  ÚNICA EXCEPCIÓN — 'AL CIERRE' / 'ANUAL' / 'meta del año': solo si el usuario pregunta EXPLÍCITAMENTE por la proyección o meta AL CIERRE del año ('venta estimada al cierre 2026', 'cuánto falta para la meta anual', '% del presupuesto anual ya logrado') va el pronóstico del año completo (sf.año='<año>' sin filtro de mes) contra el real a la fecha. Ante la duda: ALINEA.\n" .
            ...
            "  ⚠⚠⚠ REGLA ABSOLUTA — PROHIBIDO MOSTRAR DATOS EN html CUANDO HAY sql:\n" .
            "  → PROHIBIDO etiqueta <table> dentro del html (cero excepciones).\n" .
            "  → PROHIBIDO <ul>/<ol> con valores concretos (ej: '<li>Monica: 1200 kg</li>').\n" .
            "  → PROHIBIDO cualquier cifra concreta (kilos, USD, COP, porcentajes, cantidades, totales) — no sabes los valores hasta que el frontend ejecute el sql.\n" .
            "  → PROHIBIDO nombres de ejecutivas/clientes/productos asociados a números (ej: 'Monica lleva 1200 kg').\n" .
            "  → Si sientes la tentación de 'dar ejemplo' o 'mostrar el total' — NO LO HAGAS. Esos números los inventas tú = alucinación.\n" .
            "  → El html es SOLO texto explicativo de QUÉ hace la query: variables que compara, filtros aplicados, período, agrupaciones. El frontend pinta la tabla con los resultados reales.\n" .
            "  → BIEN: <p>Comparativa de kilos pedidos (OC creadas en abril 2026) vs kilos pronosticados (modelo manual) agrupada por ejecutiva.</p>\n" .
            "  → MAL: <p>La tabla muestra: Monica 1200 kg, Claudia 3500 kg, total 13,200 kg.</p>\n" .
            "  → MAL: <table>...</table> dentro del html con cualquier dato.\n" .
            "  → Excepción única: si sql=null (pregunta sin datos), puedes responder normal en html sin tabla.\n" .
...
SELECT par.dispatch_date, c.client_name, p.code, p.product_name,
       ...
       COALESCE(
         NULLIF(CASE WHEN (par.trm + 0) BETWEEN 3200 AND 10000 THEN (par.trm + 0)
                     ...
                     ELSE NULL END, 0),
         (SELECT td.value FROM trm_daily td WHERE td.date <= par.dispatch_date AND td.value >= 3200 ORDER BY td.date DESC LIMIT 1),
         4000
       ) AS trm_eff,
...
EOT;
    ...
    private function getClientsCatalogForPrompt(): string
    {
        ...
        return "CATÁLOGO DE CLIENTES (" . count($names) . " totales):\n"
            ...
            . "- ⚠⚠⚠ REGLA INVIOLABLE: SIEMPRE usa `c.client_name LIKE '%FRAGMENTO%'` (con wildcards `%`). NUNCA `c.client_name = 'fragmento'` ni `LIKE 'fragmento'` (sin %).\n"
            . "  → Los nombres reales son largos (ej. 'Bouquet Aromas Y Fragancias S.a.s.', 'LABORATORIOS BEAUTIK sa'). El usuario casi nunca escribe el nombre completo.\n"
            . "  → Casos confirmados de error: el usuario dijo 'Bouquet' y la query falló porque filtró `= 'Bouquet'` o `LIKE 'Bouquet'`. La correcta es `LIKE '%Bouquet%'`.\n"
            . "- El collation utf8mb4_unicode_ci hace LIKE case-insensitive, así que `LIKE '%bouquet%'` = `LIKE '%Bouquet%'` = `LIKE '%BOUQUET%'`. NO necesitas LOWER() ni UPPER().\n"
            ...
            . "  → Ejemplos:\n"
            . "     'ventas de Beautik'   → c.client_name LIKE '%Beautik%'   (es LABORATORIOS BEAUTIK sa)\n"
            . "     'NEW WINs de Bouquet' → c.client_name LIKE '%Bouquet%'   (es Bouquet Aromas Y Fragancias S.a.s.)\n"
            . "     'cartera de Jabones El Tigre' → c.client_name LIKE '%Jabones El Tigre%'\n"
            . "     'kilos a Quimisense'  → c.client_name LIKE '%Quimisense%'\n"
            . "- Si el fragmento del usuario es ambiguo (matchea varios clientes), usa el más distintivo o pregunta para desambiguar.\n"
    ...
    private function buildStats(string $startDate, string $endDate): array
    {
        ...
        foreach ($orderProducts->groupBy('order_id') as $orderId => $items) {
            ...
            if (!$isSample) {
                ...
                $trm   = 4000.0;
                if ($realPartial && !empty($partialTrm) && $partialTrm >= 3200) {
                    $trm = (float) $partialTrm;
                } elseif (isset($trmData[$plannedDate])) {
                    $trm = (float) $trmData[$plannedDate];
                } elseif (!empty($first->order_trm) && $first->order_trm >= 3200) {
