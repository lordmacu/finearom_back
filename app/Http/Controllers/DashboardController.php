...
class DashboardController extends Controller
{
    ...
    private function getTrmForDateWithPartial($date, $partialTrm, $orderTrm, $trmData, $sourceType, &$trmUsageStats)
    {
        ...
        if ($sourceType === 'partial_real' && !empty($partialTrm) && $partialTrm >= 2500) {
        ...
        if (!empty($orderTrm) && $orderTrm >= 2500) {
     ...
     *   TRM: po.trm ≥ 2500 → trm_daily en fecha creación → 4000
     *
     * Despachos (valor real):
     *   Partials tipo 'real' con dispatch_date en el período.
     *   TRM: partials.trm ≥ 2500 → trm_daily en fecha despacho → 4000
        ...
        $realDispRows = DB::table('partials as pt')
            ...
            ->selectRaw("
                ...
                SUM(
                    ...
                    (CASE
                        WHEN CAST(REPLACE(REPLACE(COALESCE(pt.trm,'0'), '$', ''), ',', '') AS DECIMAL(15,2)) >= 2500
        ...
        $rows = DB::table('siigo_sales as ss')
            ...
            ->selectRaw("
                ...
                SUM(
                    ss.valor /
                    (CASE
                        WHEN lpt.real_trm IS NOT NULL AND lpt.real_trm >= 2500 THEN lpt.real_trm
                        WHEN po.trm IS NOT NULL AND po.trm >= 2500 THEN po.trm
                        ELSE 4000
                    END)
                ) as value_usd
            ")
